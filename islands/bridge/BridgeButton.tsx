...
function acknowledge() { acknowledgeGate.value.resolve() }
function cancel() { acknowledgeGate.value.reject('cancelled') }
...
async function bridge() {
  ...
  if (
    ...
    * (1 - state.slippage.value / 100)
    ...
    .catch(() => { throw new Error('user declined bridge') })
  ...
  await switchChain().catch((e:{ code:number }) => e.code == 4902
    ? addChain().then(switchChain) : {}
  )
        ...
        <div>
          Trade slippage ({
             ...
             * (1
              - Number(state.to.input.string.value)
              / Number(state.from.input.string.value)))
          }%) exceeds specified slippage ({
            state.slippage.value
          }%). This bridge might never complete. To continue, press 'Continue' below.
        </div>
