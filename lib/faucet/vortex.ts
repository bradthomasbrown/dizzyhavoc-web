    ...
    async updater() {
      ...
      globalThis.addEventListener(
        ...
        (event: EIP6963AnnounceProviderEvent) => {
          p6963s.set(event.detail.info.uuid, { ...event.detail });
        },
      );
      ...
      if (navigator.maxTouchPoints > 0) {
        ...
        const gate = new Gate<void>();
        setTimeout(() => {
          const provider = MMSDK.getProvider() as
            | undefined
            | P1193 & Partial<P5749>;
          if (!provider) {
            gate.resolve();
      ...
      else if (p6963s.size == 1) {
        provider = (p6963s.values().next().value as P6963).provider; //this.operator.set()
