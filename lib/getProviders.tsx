...
import { Blockie, Choice, dzkv, randomUuid } from 'lib'
import { Signal } from "@preact/signals";
import { JSX } from "preact/jsx-runtime";
import { Which } from 'islands.common'
...
function g1193(): G1193 { return globalThis; }
...
async function onPick(p6963:P6963) {
  ...
  p6963.provider.on(
    'chainChanged',
    s => z.string().transform(s => Number(BigInt(s))).parseAsync(s)
...
async function onChainChanged(chainId:number) {
  ...
  if (!chainmap.has(chainId)) {
    ...
    if (chain instanceof Error) { console.error(chain); return }
    chainmap.set(chainId, chain)
  }
  
  const chain = chainmap.get(chainId)!
  const key = ['chains', 'p6963']
  if (!dzkv.ensure<Signal<Chain>>(key, new Signal(chain)))
    dzkv.get<Signal<Chain>>(key)!.value = chain
