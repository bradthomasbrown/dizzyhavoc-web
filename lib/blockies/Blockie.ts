...
export class Blockie {
    ...
    base64() {
        ...
        function nextRandomNumber() {
            ...
            randomBuffer[3] = randomBuffer[3] ^ (randomBuffer[3] >> 19) ^ t ^ (t >> 8)
        ...
        function nextRandomRgb() {
            ...
            const l = (nextRandomNumber() + nextRandomNumber() + nextRandomNumber() + nextRandomNumber()) * 25 / 100
        ...
        const logicalScreenDescriptor = new Uint8Array([
            (8 * this.scale) & 0xff, (8 * this.scale) >> 8, // width, 2 bytes
            (8 * this.scale) & 0xff, (8 * this.scale) >> 8, // height, 2 bytes
            //GBBBSZZZ
        ...
        const imageDescriptor = new Uint8Array([
            ...
            (8 * this.scale) & 0xff, (8 * this.scale) >> 8, // width
            (8 * this.scale) & 0xff, (8 * this.scale) >> 8, // height
            //LISRRZZZ
        ...
        for (let i = 0; i < indexStream.length; i++) {
            ...
            for (let j = 0; j < this.scale; j++)
                for (let k = 0; k < this.scale; k++)
                    scaledIndexStream[scaledX + k + (scaledY + j) * scaledL] = indexStream[i]
        }
        ...
        while (bytes.length) dataSubBlocks.push(new Uint8Array([Math.min(bytes.length, 255), ...bytes.splice(0, 255)]))

        const imageData = new Uint8Array([lzwMinimumCodeSize, ...dataSubBlocks.flatMap(dataSubBlock => [...dataSubBlock]), 0])
