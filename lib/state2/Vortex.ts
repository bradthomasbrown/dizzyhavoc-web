...
export type TStateSet = (
  this: TStateSetContext,
  value: unknown,
) => Error | void;
...
export class TStateOperator {
  ...
  get(key?: string) {
    return this.tState[key ?? this.key];
  }
  ...
  errors(dependencies: string[]): Error[] {
    return dependencies.map((key) => this.tState[key]).filter((value) =>
...
export class Vortex<F extends VortexFlows, D extends VortexData> {
  ...
  constructor(flows: F, data: D) {
    ...
    this.uState = Object.fromEntries(
      Object.entries(data).map(([k]) => [k, new Signal(undefined)]),
    ) as UStateSpecific<D>;
  }
