import { overrides, schemas } from "../internal.ts";
...
export async function query({ id }: { id: number }): Promise<Error | Chain> {
  ...
  const chain = override ? null : await fetch(
    `https://cdn.jsdelivr.net/gh/ethereum-lists/chains/_data/chains/eip155-${id}.json`,
  )
