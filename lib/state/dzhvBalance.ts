...
async function updateDzhvBalance({ tState, signal }:UpdaterOpts) {
    ...
    if (tState.addresses instanceof Error) { tState.dzhvBalance = tState.addresses; return }
    if (tState.dzhv instanceof Error) { tState.dzhvBalance = tState.dzhv; return }
    if (tState.height instanceof Error) { tState.dzhvBalance = tState.height; return }
    if (tState.rpc instanceof Error) { tState.dzhvBalance = tState.rpc; return }
    ...
    if (!address) { tState.dzhvBalance = new Error('tState.addresses empty'); return }

    // get and parse
    const dzhvBalance = await ejra.call(
        tState.rpc,
        { to: tState.dzhv.address, input: `0x70a08231${address.substring(2).padStart(64, '0')}` },
        tState.height
