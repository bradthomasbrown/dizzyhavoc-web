...
const pairSchema = z.object({ chainId: z.string(), priceUsd: z.string().transform(Number) }).passthrough()
...
export const fromPrice: VortexDatum = {
  ...
  async updater() {
    ...
    const lazy:Lazy<Error|z.infer<typeof responseSchema>> = async () => await fetch("https://api.dexscreener.com/latest/dex/tokens/0x3419875B4D3Bca7F3FddA2dB7a476A79fD31B4fE")
    ...
    const response = await toad.feed(snail).catch(reason => new Error(reason))
    if (response instanceof Error) { this.operator.set(response); return }
