...
export class Snail<T> {
  ...
  constructor({ lazy, signal }:{ lazy:() => Promise<T>, signal?:AbortSignal }) {
...
export class Ejra {
  ...
  balance(this:Ejra, url:string, ...p:[address:string, tag:Tag, signal?:AbortSignal]) {
    const request = { method: 'eth_getBalance', schema: schema.quantity }
    return this.#call(url, { ...request, params: p })
  }

  slot(this:Ejra, url:string, ...p:[address:string, slot:bigint, tag:Tag, signal?:AbortSignal]) {
    const request = { method: 'eth_getStorageAt', schema: schema.string }
    return this.#call(url, { ...request, params: p })
  }

  nonce(this:Ejra, url:string, ...p:[address:string, tag:Tag, signal?:AbortSignal]) {
    const request = { method: 'eth_getTransactionCount', schema: schema.quantity }
    return this.#call(url, { ...request, params: p })
  }

  code(this:Ejra, url:string, ...p:[address:string, tag:Tag, signal?:AbortSignal]) {
  ...
  call(this:Ejra, url:string, ...p:[txCallObject:TxCallObject, tag:Tag, signal?:AbortSignal]) {
    const request = { method: 'eth_call', schema: schema.string }
    return this.#call(url, { ...request, params: p })
  }

  estimateGas(this:Ejra, url:string, ...p:[txCallObject:Partial<TxCallObject>, signal?:AbortSignal]) {
    const request = { method: 'eth_estimateGas', schema: schema.quantity }
    return this.#call(url, { ...request, params: p })
  }

  receipt(this:Ejra, url:string, ...p:[hash:string, signal?:AbortSignal]) {
    const request = { method: 'eth_getTransactionReceipt', schema: schema.receipt }
    return this.#call(url, { ...request, params: p })
  }
  ...
  >(this:Ejra, url:string, request:E) {
    ...
    const lazy = async ():Promise<z.infer<E['schema']>> => {
      ...
      if ('error' in jraResponse) throw new Error('jraErr', { cause: jraResponse })
...
export const toad = new Toad(250)
